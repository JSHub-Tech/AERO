...
import { Plane, Lock, Mail, ArrowRight, AlertTriangle, ShieldCheck } from 'lucide-react';
...
export default function Auth() {
      ...
      <div className="min-h-screen bg-[#F8F9FA] relative flex flex-col pt-[100px]">
        ...
        <div className="flex-grow flex items-center justify-center px-6 relative z-10 pb-20">
          <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 p-10 max-w-lg w-full text-center">
            <div className="w-20 h-20 bg-[#004F30]/10 rounded-full flex items-center justify-center mx-auto mb-6 text-[#004F30]">
              <ShieldCheck size={40} />
            </div>
            ...
            <div className="flex flex-col gap-4">
              <button 
                ...
                className="w-full bg-[#004F30] hover:bg-[#1C2B22] text-white font-black py-4 rounded-xl transition-all shadow-lg shadow-[#004F30]/20 text-xs uppercase tracking-widest flex items-center justify-center gap-2"
              >
              ...
              {user.role === 'admin' && (
                <button 
                  ...
                  className="w-full bg-red-50 hover:bg-red-600 hover:text-white text-red-600 font-black py-4 rounded-xl transition-all text-xs uppercase tracking-widest border border-red-100"
                >
  ...
  const handleSubmit = async (e) => {
    ...
    if (email && password) {
      ...
      // Right now the API only has login fully wired up. 
      // If authMode === 'register', ideally we'd call a register API.
      // For now we just call login for both to simulate the flow.
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] relative flex flex-col pt-[100px]">
      ...
      <div className="flex-grow flex items-center justify-center px-6 relative z-10 pb-20">
        ...
        <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 bg-white rounded-[40px] shadow-[0_30px_60px_rgba(0,0,0,0.08)] overflow-hidden border border-gray-100">
          ...
          <div className="hidden md:flex flex-col justify-between bg-[#1C2B22] p-12 text-white relative overflow-hidden">
            ...
            <div className="absolute -bottom-20 -right-20 w-96 h-96 bg-[#004F30] rounded-full blur-[100px] opacity-40"></div>
            
            <div className="relative z-10">
              <img src="/logo.png" alt="AERO" className="h-12 w-auto mb-6 brightness-0 invert" />
              <h1 className="text-5xl font-black tracking-tighter leading-none mb-4">AERO<br/>GLOBAL<br/>NETWORK</h1>
          ...
          <div className="p-10 md:p-14 flex flex-col justify-center">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-6">
              ...
              <button disabled={isSubmitting} type="submit" className="mt-6 w-full bg-[#004F30] hover:bg-[#1C2B22] disabled:bg-gray-400 text-white font-black py-4 rounded-2xl transition-all shadow-xl shadow-[#004F30]/20 text-xs uppercase tracking-widest flex items-center justify-center gap-2 group">
                {isSubmitting ? 'Authenticating...' : (authMode === 'login' ? 'Authenticate' : 'Register Account')}
